...
public class FMODStudioFirstPersonFootsteps : MonoBehaviour
{
    ...
    public LayerMask steppableLayer;
    ...
    {
        if (Physics.Raycast(transform.position, Vector3.down, out hit, RayDistance, steppableLayer))                                 // A raycast is fired down, from the position that the player is curenntly standing at, traveling as far as we decide to set the 'RayDistance' variable to. Infomration about the object it comes into contact with will then be stored inside the 'hit' variable for us to access.
        {
    ...
    {
        Physics.Raycast(transform.position, Vector3.down, out hit, RayDistance, steppableLayer);    // First, a raycast is fired from the location of the player, downwards towards the ground. It travels for as far as we set the 'RayDistance' variable to in UNity's Inspector and then stores information about whatever it comes into contact with inside the 'hit' variable. 
